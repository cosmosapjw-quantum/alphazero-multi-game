#!/usr/bin/env python3
"""
Interactive Go game against AlphaZero AI.

Usage:
    python play_go.py [options]

Options:
    --model MODEL       Path to model file
    --size SIZE         Board size (default: 9)
    --simulations SIMS  Number of MCTS simulations (default: 800)
    --threads THREADS   Number of threads (default: 4)
    --chinese           Use Chinese rules (default: Japanese rules)
"""
...
class GoGUI:
    """
    GUI for playing Go against AlphaZero.
    """
    
    def __init__(self, master, args):
        self.master = master
        master.title("AlphaZero Go")
        
        # Parse arguments
        self.board_size = args.size
        self.simulations = args.simulations
        self.threads = args.threads
        self.use_chinese = args.chinese
    ...
    def setup_game(self):
        ...
        self.game_state = az.createGameState(
            az.GameType.GO, self.board_size, self.use_chinese
        )
        ...
        if self.model_path:
            try:
                self.network = az.createNeuralNetwork(
                    self.model_path, az.GameType.GO, self.board_size
                )
    ...
    def create_widgets(self):
        ...
        # Pass button
        self.pass_button = tk.Button(self.control_frame, text="Pass", 
    ...
    def pass_move(self):
        ...
        # In Go, passing is typically the last action
        pass_action = self.board_size * self.board_size
        
        # Check if pass is legal
        if not self.game_state.isLegalMove(pass_action):
            self.status_text.set("Cannot pass right now!")
            return
        
        # Make the pass move
        self.game_state.makeMove(pass_action)
        
        # Update MCTS tree
        self.mcts.updateWithMove(pass_action)
...
def parse_args():
    parser = argparse.ArgumentParser(description="AlphaZero Go")
    ...
    parser.add_argument("--chinese", action="store_true",
                        help="Use Chinese rules (default: Japanese rules)")
